...
def loss_all(xr: torch.Tensor, xb: torch.Tensor, u_theta, phi_eta, alpha, device):
    ...
    _grad_2_u_theta = grad(torch.sum(_grad_u_theta).squeeze(), xr, create_graph=True)[0]
    _grad_2_u_theta = _grad_2_u_theta.squeeze()
    
    _out_phi_eta = phi_eta(xr).squeeze()
    #_grad_phi_eta = grad(_out_phi_eta, xr, create_graph=True)[0]
    
    #print(_grad_u_theta.shape, _grad_phi_eta.shape)
    ...
    t1 = _grad_2_u_theta * _out_phi_eta    # norm
    I = t1 - g(xr) * _out_phi_eta
    # <<< PDE-specific: calculate for I (integrand) <<<
